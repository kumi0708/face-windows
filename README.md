...
| **ミラー** | カメラ画像とデスクトップを **1:1** に対応させ、顔・目・鼻・口の窓を「カメラ上でその部位が写っている位置・大きさ」に置く。既定の動き（**ずれたら生成**）では窓は動かず、顔の位置・大きさがずれるたびにその時点のミラー位置へ新しい窓を生成し、置いていかれた窓はその場に残ってフェードして消える（顔を動かした跡に窓が並ぶ）。「窓が部位を追いかける」動きにも切り替えられる。デスクトップ上に窓で組み立てた自分の顔が現れ、顔を動かすと窓も一緒に動く・近づくと大きくなる。窓はミラーの位置にだけ表示し、勝手に飛び回る窓は出さない（口/頭の反応BURSTは既定 OFF）。オプションで、遅れてついてくる残像の窓（過去の映像）を重ねたり、反応BURSTを ON にしたりできる |

管理画面の「表示モード」で切り替える。PRESETS の「ミラー（顔を再構成）」を適用すると、ミラー向けの設定（切り抜き 360px、回転補正 OFF など）がまとめて入る。
ミラーの設定（DISPLAY タブ）：カメラ→画面の合わせ方（比率を保って画面を埋める／収める／引き伸ばす）、窓の大きさ、残像の数・遅れ・濃さ、切り抜き解像度。
窓は「部位をつかんで追いかける」動きになるよう、検出枠に One Euro フィルタ（止まっている時のブレを抑え、動くと素早く追従）をかけ、検出（30fps）の間は描画（60fps）で補間して動かす（TRACKING の「ブレ補正」、DISPLAY の「窓の動きのなめらかさ」で調整）。
...
- Windows 10/11（検証済み）
- macOS 13 以降・Apple Silicon（M1 以降）（対応コードあり・**実機未検証**。MediaPipe の Mac 版が Apple Silicon 専用のため Intel Mac は不可）
...
run.bat
```
macOS は `./run.sh`。初回はカメラの使用許可を求められる（許可しなかった場合は「システム設定 → プライバシーとセキュリティ → カメラ」でターミナルを許可して再起動）。
...
| DISPLAY | 表示モード（増殖／ミラー）とミラーの設定、描画方式（疑似／OS実／混合）、実ウィンドウ上限、表示モニター、スタイル（Win11 ライト／ダーク／macOS風／Win95風／フレームレス）、不透明度、影、スナップショット窓・ディレイ窓・残像の割合 |
...
- macOS 対応は実機で未検証。macOS ではグローバル緊急停止キーなし、カメラ名は番号表示、OS 実ウィンドウの移動は1枚ずつ（Windows の一括移動 API がないため遅くなる可能性がある）。
