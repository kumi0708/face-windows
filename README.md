...
| **ミラー** | カメラ画像とデスクトップを **1:1** に対応させ、顔・目・鼻・口の窓を「カメラ上でその部位が写っている位置・大きさ」に置く。デスクトップ上に窓で組み立てた自分の顔が現れ、顔を動かすと窓も一緒に動く・近づくと大きくなる。各部位には遅れてついてくる残像の窓（過去の映像）を重ねられる。BURST や口/頭の反応で生まれた窓は増殖モードと同じように動き回る |
...
設定の検証、左右・反転の座標、切り抜きの向き、上限を超えて増えないこと、全部位 OFF で生成しないこと、画面外へ消えないこと、寿命・見失い時の消滅、自動負荷調整、実ウィンドウ上限、口の BURST、PAUSE、ミラー表示の座標対応・部位ごとの窓・残像の遅れ・見失い時の消去・モード切替を検証する（26 件）。
